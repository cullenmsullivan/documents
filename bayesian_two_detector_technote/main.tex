...
Minimal post-processing first involves merging the outputs using something like \texttt{hadd\_cafana}. However, managing the sheer number of sub-predictions alone is challenging. \texttt{TwoDet/\-shell\_\-functions\-.sh} contain terminal-level bash functions to assist in finding problems with grid outputs and with merging them. To find small files (which are indicated by smaller file size than successful jobs), use \texttt{get\_bad\_output}. As an example,
\begin{verbatim}
...
To query what your new systematically-shifted samples look like, use \texttt{diagnositc\-/Plot\-Syst\-Pulls\-.C} to plot $+1\sigma$ and $-1\sigma$ variations for every systematic you loaded for every recipe.
